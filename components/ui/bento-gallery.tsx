...
> = ({ imageItems, title, description }) => {
  ...
  // GSAP Draggable: glisser horizontalement pour faire défiler (avec élan).
  useEffect(() => {
    ...
    const build = () => {
      const minX = Math.min(0, container.offsetWidth - track.scrollWidth - 32)
      // Garde la position dans les bornes après un redimensionnement.
      gsap.set(track, {
        x: gsap.utils.clamp(minX, 0, Number(gsap.getProperty(track, "x")) || 0),
      })
      draggable?.kill()
  ...
  return (
    ...
    >
      ...
      <div ref={containerRef} className="relative mt-12 w-full overflow-hidden">
        <div ref={trackRef} className="w-max cursor-grab active:cursor-grabbing">
          <motion.div
            ref={gridRef}
